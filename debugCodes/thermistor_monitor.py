...
def update_display(temperatures, measurement_count, errors, dht_temp, dht_humidity):
    ...
    for i in range(10):
        ...
        if math.isnan(temp_right):
            ...
            temp_str_right = (
                f"{temp_right:7.3f}°C"  # 3 decimales para mostrar precisión
            )
...
def print_calibration_info():
    ...
    for i in range(10):
        ...
        print(
            f"  {left_id}: {left_res:5d}Ω ({left_res/1000:.2f}kΩ)    "
    ...
    try:
        while running:
            ...
            if current_time - last_measurement_time >= 1.0:
                ...
                current_errors = sum(
                    1 for temp in temperatures.values() if math.isnan(temp)
                )
                if current_errors > 0:
                    error_count += 1
        ...
        if measurement_count > 0:
            ...
            if dhtDevice is not None:
                dht_success_rate = (
                    (measurement_count // 3 - dht_error_count)
                    / (measurement_count // 3)
                ) * 100
