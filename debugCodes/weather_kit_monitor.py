...
def start_rain_polling():
    ...
    try:
        ...
        print(
            f"✓ Lluvia (GPIO {RAIN_SENSOR_PIN}) listo para polling, estado inicial: {rain_last_state} ({'HIGH' if rain_last_state else 'LOW'})"
        )
    except Exception as e:
...
def initialize_hardware():
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(
                f"Intento {attempt + 1}/{max_retries} - Error inicializando ADS1115: {e}"
            )
    ...
    # Anemómetro Setup
    try:
        anemometer = Button(ANEMOMETER_PIN, pull_up=True, bounce_time=0.01)
...
def update_display(weather_data, measurement_count, error_count):
    ...
    print(
        f"Medición #{measurement_count:05d} - {timestamp} - Errores: {error_count}     "
    )
...
def print_station_info():
    ...
    print(
        f"Anemómetro (GPIO {ANEMOMETER_PIN}):      {KPH_PER_COUNT_PER_SEC} km/h por pulso/segundo"
    )
    ...
    try:
        while running:
            ...
            if current_time - last_measurement_time >= 1.0:
                ...
                if (
                    weather_data["wind_angle"] is not None
                    and weather_data["wind_direction"] is not None
                ):
                ...
                if (
                    last_valid_wind_speed is None
                    and current_time - last_wind_update > 5
                ):
                    current_errors += 1
        ...
        try:
            ...
            try:
                if (
                    "rain_poll_thread" in globals()
                    and rain_poll_thread
                    and rain_poll_thread.is_alive()
                ):
