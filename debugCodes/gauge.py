...
def rain_gauge_rpi_gpio():
    ...
    def rain_pulse(channel):
        nonlocal rain_count
        rain_count += 1
        print(f"Pulso detectado! Total: {rain_count} -> {rain_count * MM_PER_TICK:.3f} mm")
    ...
    try:
        ...
        print(f"✓ Estado inicial del pin: {initial_state} ({'HIGH' if initial_state else 'LOW'})")
        
        # Añadir detección de flanco con manejo de errores
        try:
            GPIO.add_event_detect(RAIN_PIN, GPIO.FALLING, callback=rain_pulse, bouncetime=300)
