...
def setup_ina(i2c, address):
    ...
    try:
        ...
        success_avg = _try_set(
            "averaging_count", s, AVG_TARGET, fallback=7
        )  # 7 suele ser 1024
        success_bus = _try_set(
            "bus_voltage_conv_time", s, CT_TARGET_US, fallback=5
        )  # 5 suele ser ~1052us
        ...
        actual_ct = getattr(
            s, "bus_voltage_conv_time", getattr(s, "conversion_time_bus", "N/A")
        )
...
def setup_display():
    ...
    print(
        f"║          Rshunt: {RSHUNT_OHMS}Ω - Imax: {IMAX_AMPS}A - AVG: {AVG_TARGET}                   ║"
    )
...
def print_calibration_info():
    ...
    print(
        f"Direcciones I2C:          {', '.join(f'0x{addr:02X}' for addr in ADDRESSES)}"
    )
...
def main():
    ...
    try:
        ...
        if active_sensors:
            print(
                f"Sensores activos durante la sesión: {', '.join(f'0x{addr:02X}' for addr in active_sensors)}"
            )
